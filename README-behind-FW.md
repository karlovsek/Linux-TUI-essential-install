...
On the remote clone the repository
```ALL_PROXY=socks5h://127.0.0.1:1080 git clone https://github.com/karlovsek/Linux-TUI-essential-install```

# Download nix installation script and patch it so it will work over proxy socks5
```curl -x socks5h://localhost:1080 -L https://nixos.org/nix/install > nix-install.sh &&
   git apply nix-install-proxy-curl-patch.patch && chmod +x nix-install-proxy-curl-patch.sh```
